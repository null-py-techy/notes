...
```shell-session
ninjathebox98w1@htb[/htb]$ ./linpeas.sh
...SNIP...
...
```shell-session
ninjathebox98w1@htb[/htb]$ sudo -l
...
```shell-session
ninjathebox98w1@htb[/htb]$ sudo su -
...
```shell-session
ninjathebox98w1@htb[/htb]$ sudo -u user /bin/echo Hello World!

    Hello World!
```
Once we find a particular application we can run with `sudo`, we can look for ways to exploit it to get a shell as the root user. [GTFOBins](https://gtfobins.github.io/) contains a list of commands and how they can be exploited through `sudo`. We can search for the application we have `sudo` privilege over, and 
if it exists, it may tell us the exact command we should execute to gain root access using the `sudo` privilege we have.
...
```shell-session
ninjathebox98w1@htb[/htb]$ vim id_rsa
ninjathebox98w1@htb[/htb]$ chmod 600 id_rsa
ninjathebox98w1@htb[/htb]$ ssh root@10.10.10.10 -i id_rsa
...
```shell-session
ninjathebox98w1@htb[/htb]$ ssh-keygen -f key
...
```shell-session
ninjathebox98w1@htb[/htb]$ ssh root@10.10.10.10 -i key

root@remotehost# 
```
![](Pasted%20image%2020250920161521.png)