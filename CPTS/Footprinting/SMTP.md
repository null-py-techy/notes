...
Enumerate the SMTP service and submit the banner, including its version as the answer. - `nmap -p25 -sC -sV 10.129.42.195` - `InFreight ESMTP v2.11` 

Enumerate the SMTP service even further and find the username that exists on the system. Submit it as the answer. - `Fuck use metasploit` - `msfconsole`, `search smtp_enum`, `use 0`, `options`, `set rhosts <ip>`, `use htb resource wordlist`, `unzip, set USER_FILE to be that file`, `exploit` - `robin` 

Figure out the exact organization name from the IMAP/POP3 service and submit it as the answer. - `openssl s_client --connect 10.129.184.66:imaps` - `InlaneFreight Ltd` 

What is the FQDN that the IMAP and POP3 servers are assigned to? - `dev.inlanefreight.htb` 

Enumerate the IMAP service and submit the flag as the answer. (Format: HTB{...}) - `HTB{roncfbw7iszerd7shni7jr2343zhrj}` 

What is the customized version of the POP3 server? - `openssl s_client --connect 10.129.184.66:pop3s` - `InFreight POP3 v9.188`

What is the admin email address? - `openssl s_client --connect 10.129.184.66:imaps` - `A1 LOGIN robin robin` - `1 LIST "" *` - `1 SELECT DEV.DEPARTMENT.INT` - `A3 FETCH 1 BODY[]` - `devadmin@inlanefreight.htb` - Note these A1, A2, A3, 1 are all increments and can be anythings help with tracking. 22) Try to access the emails on the IMAP server and submit the flag as the answer. (Format: HTB{...}) - `HTB{983uzn8jmfgpd8jmof8c34n7zio}`