...
```shell-session
ninjathebox98w1@htb[/htb]$ dig soa www.inlanefreight.com
...
```shell-session
ninjathebox98w1@htb[/htb]$ dig ns inlanefreight.htb @10.129.14.128
...
```shell-session
ninjathebox98w1@htb[/htb]$ dig CH TXT version.bind 10.129.120.85
...
```shell-session
ninjathebox98w1@htb[/htb]$ dig any inlanefreight.htb @10.129.14.128
...
```shell-session
ninjathebox98w1@htb[/htb]$ dig axfr inlanefreight.htb @10.129.14.128
...
```shell-session
ninjathebox98w1@htb[/htb]$ dig axfr internal.inlanefreight.htb @10.129.14.128
...
```shell-session
ninjathebox98w1@htb[/htb]$ for sub in $(cat /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt);do dig $sub.inlanefreight.htb @10.129.14.128 | grep -v ';\|SOA' | sed -r '/^\s*$/d' | grep $sub | tee -a subdomains.txt;done
...
```shell-session
ninjathebox98w1@htb[/htb]$ dnsenum --dnsserver 10.129.14.128 --enum -p 0 -s 0 -o subdomains.txt -f /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt inlanefreight.htb
...
 ->
 ``` dig any inlanefreight.htb @10.129.62.79

; <<>> DiG 9.20.13 <<>> any inlanefreight.htb @10.129.62.79
;; global options: +cmd
;; Got answer:
...
;; OPT PSEUDOSECTION:
; EDNS: version: 0, flags:; udp: 4096
; COOKIE: 5ed981459aafd0e70100000068d7da740627a04197c51dd5 (good)
;; QUESTION SECTION:
;inlanefreight.htb.		IN	ANY
...
;; Query time: 307 msec
;; SERVER: 10.129.62.79#53(10.129.62.79) (TCP)
;; WHEN: Sat Sep 27 18:22:08 +0545 2025
;; MSG SIZE  rcvd: 437
 ```

Identify if its possible to perform a zone transfer and submit the TXT record as the answer. (Format: HTB{...})
->
```dig axfr internal.inlanefreight.htb  @10.129.62.79

; <<>> DiG 9.20.13 <<>> axfr internal.inlanefreight.htb @10.129.62.79
;; global options: +cmd
internal.inlanefreight.htb. 604800 IN	SOA	inlanefreight.htb. root.inlanefreight.htb. 2 604800 86400 2419200 604800
internal.inlanefreight.htb. 604800 IN	TXT	"MS=ms97310371"
...
;; Query time: 308 msec
;; SERVER: 10.129.62.79#53(10.129.62.79) (TCP)
;; WHEN: Sat Sep 27 18:22:45 +0545 2025
...
 ->```dnsenum --dnsserver 10.129.62.79 -o subdomains.txt -f /usr/share/seclists/Discovery/DNS/fierce-hostlist.txt dev.inlanefreight.htb
 we can find win2k.dev.inlanefreight.htb inside this output so this is  the answer.
 ```